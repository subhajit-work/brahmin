...
function validateForm(form, submission, fancy) {
                ...
                if (element.name.indexOf('mobile') >= 0) {
                    if (!IsmobileValid(element)) {
                        ...
                        if (show == 0) {
                            // element.focus();
                            show = 1;
                            // jQuery(str).removeClass('haserror');
                        }
                        displayError(element);
                        retval = false;
                    }
    ...
    if (submission == 'ajax' && retval) {
        //jQuery("#"+fancy).html('<img src="'+base_url+'images/loading.gif" style="margin-top:5px">').show();
        jQuery.ajax({
            ...
            success: function(data) {
                if (data.message == "success") {
                    if (data.alertmsg != "no" && data.alertmsg != "") {
                        jQuery("#" + fancy).html('');
                        alert(data.alertmsg);
                    }
                    if (data.msg != "no") {
...
function Validate(id) {
    ...
    for (i = 0; i < elements.length; i++) {
        ...
        if (element.name.indexOf('mobile') >= 0) {
            if (!IsmobileValid(element))
                retval = showError(element);
            // console.log("retval222222 >", retval);
        }
        // phobe no
    }
    return retval;
...
function value(element) {
    ...
    if (jQuery(element).attr('multiple') == true) {
        ...
        val = jQuery(element).val().toString();
    return val;
}
...
// phone no..
function IsmobileValid(str, name) {
    // console.log("phone validation work");
    // console.log("str ==>>", str);
    // console.log("name ==>>", name);
 
    var phoneno=/^\(?([0-9]{3})\)?[-. ]?([0-9]{3})[-. ]?([0-9]{4})$/;if(str.value.match(phoneno))
    {
        return true;
    }
}
    ...
    jQuery.ajax({
        ...
        success: function(data) {
            if (data == 1)
                ...
                jQuery(form).find('.alert').html('<div class="ui-state-error ui-corner-all" style="padding: 0 .7em;"><p><span class="ui-icon ui-icon-alert" style="float: left; margin-right: .3em;"></span><strong>Alert:</strong>  ' + data + '</p></div>');
                jQuery(form).find('fieldset').show();
                jQuery(form).find('.loading').hide();
            }
        },
        error: function(data) {
            jQuery(form).find('.alert').html('<div class="ui-state-error ui-corner-all" style="padding: 0 .7em;"><p><span class="ui-icon ui-icon-alert" style="float: left; margin-right: .3em;"></span><strong>Alert:</strong> Request Failed</p></div>');
            jQuery(form).find('fieldset').show();
            jQuery(form).find('.loading').hide();
        }
    });
